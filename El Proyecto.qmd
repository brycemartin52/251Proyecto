...
for(t in 2:iters){
  ...
  log.r <- sum(dnorm(refract, mu.star, sqrt(sigma2), log=T)) + 
           dnorm(mu.star, lambda, sqrt(tau2), log=T) -
           sum(dnorm(refract, mu, sqrt(sigma2), log=T)) -
           dnorm(mu, lambda, sqrt(tau2), log=T)
