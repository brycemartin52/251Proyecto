...
```{r}
# COLLECT DATA
pop1 <- c(4.6, 3.4, 3.8, 4.2, 3.5, 4.2, 3.2, 1, 3.9, 4.4, 4.8, 3.4, 4.6, 5, 4.4, 4.6, 4.3, 4.5, 2.6, 3.4, 4.8, 4.9, 3, 4.5, 4.4, 4.6, 3.8, 3.1, 4.5, 4.8)
...
mu <- 1 #-200  #more reasonable starting value: 1.5
...
for(t in 2:iters){
  ...
  log.r <- sum(dbeta(pop1, mu.star, sqrt(sigma2), log=T)) + 
           dunif((mu.star-1)/4, lambda, sqrt(tau2), log=T) -
           sum(deta(pop1, mu, sqrt(sigma2), log=T)) -
           dunif((mu-1)/4, lambda, sqrt(tau2), log=T)
